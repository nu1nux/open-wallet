...
export function Dashboard() {
  ...
  const handleEditAlias = (alias: string) => {
  ...
  const handleCreateSuccess = () => {
    ...
    // Wallet is created and unlocked, accounts will refresh via store
  };
...
function AccountRow({
  ...
  account: { address: string; name: string; alias?: string };
  symbol: string;
    ...
    <div className="flex items-center justify-between rounded-md border border-neutral-100 bg-neutral-50 px-3 py-2.5">
      <div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => onEditAlias(account.alias || '')}
